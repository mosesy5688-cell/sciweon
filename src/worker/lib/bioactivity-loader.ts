 ...
 * Returns all matching records from the pinned snapshot.
 *
 ...
 * object key is derived UNIFORMLY from ctx.object_prefix (v1: snapshots/<date>/;
 * v2: the declared prefix), so a composed request (repurposing) cannot read
 * latest more than once or cross snapshots mid-request.
 */
...
export async function loadBioactivitiesForCompound(
    ...
    try {
        const text = await fetchR2GunzippedText(bucket, `${ctx.object_prefix}bioactivities.jsonl.gz`);
