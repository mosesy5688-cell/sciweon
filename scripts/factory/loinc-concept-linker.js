/**
 * LOINC Concept Linker -- PR-UMLS-4 F3 placement orchestrator.
 *
 * NON-CONFLATION NOTE: this LOINC axis is the UMLS LNC *concept vocabulary* (lab/clinical
 * observation codes). It is DISTINCT from lib/spl-parser.js LOINC_SECTIONS (SPL
 * document-section codes -> DrugLabel fields) -- a different axis; the two never join.
 *
 * Reads the R2 LOINC bulk artifact (internal/processed/bulk/umls/<release>/
 * loinc-concepts.jsonl.zst per PR-UMLS-4), decompresses (zstd CLI), parses each JSONL line
 * SKIPPING the leading `#`-prefixed license header + blank lines, and writes a pass-through
 * copy to output/linked/loinc-concepts.jsonl for the PR-UMLS-4 SID LOINC stamper to consume.
 *
 * The record carries `cui` as an INTERNAL-ONLY cross-link round-trip anchor (consumed by a
 * future PR-4b crosslink); it is NEVER projected to the public artifact (the Cat-0 public
 * builder drops cui).
 *
 ...
 * output/linked/loinc-concepts.jsonl is ALSO the FULL internal artifact -- it is stamped in
 * place and then PROJECTED to a Cat-0 public {sid_s,sid_c,code,str} file
 * (loinc-concepts-public.jsonl) by the F3 public builder. The full file is added to
 * AGGREGATED_FILES (internal F3->F4 round-trip) but DELIBERATELY OMITTED from SNAPSHOT_FILES
 * (no cui in the public snapshot).
 *
 * Records already carry the SID-S anchor fields (anchor_payload = `LNC:<CODE>`,
 ...
 * umls-loinc-harvest.js) is the count-of-record truth; on mismatch FAIL LOUD per
...
const LOINC_OUTPUT = 'output/linked/loinc-concepts.jsonl';
...
async function main() {
    ...
    console.log(`[${LABEL}] PR-UMLS-4 LOINC F3 placement | R2 (internal/) -> ${LOINC_OUTPUT}`);
    const cursorBuf = await getR2Object(client, bucket, LOINC_CURSOR_KEY);
    ...
    console.log(`[${LABEL}] Loaded ${records.length} LOINC concepts (skipped ${headerSkipped} header line) from ${cursor.r2_data_key}`);
    ...
    writeFileSync(LOINC_OUTPUT, output, 'utf-8');

    const elapsed = Math.round((Date.now() - startMs) / 1000);
    console.log(`[${LABEL}] Wrote ${LOINC_OUTPUT} (${records.length} concepts, ${Buffer.byteLength(output)}B) in ${elapsed}s`);
