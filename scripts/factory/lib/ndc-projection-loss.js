 ...
 * @returns {{distinct_projected,distinct_fallback_only,fallback_rate_upper_bound,tty_dist,sab_dist,samples}}
 */
export function summarizeNdcProjectionLoss(ndcsProjected, ndcsFallback, meta = new Map()) {
    ...
    for (const [ndc, hits] of (ndcsFallback ?? new Map())) {
        ...
        lost++;
        for (const h of hits) {
            const tty = meta.get(h.rxcui)?.tty || 'null';
            tty_dist[tty] = (tty_dist[tty] || 0) + 1;
            sab_dist[h.sab] = (sab_dist[h.sab] || 0) + 1;
        }
    ...
    return { distinct_projected: projected, distinct_fallback_only: lost, fallback_rate_upper_bound, tty_dist, sab_dist, samples };
}
