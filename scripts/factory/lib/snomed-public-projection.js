...
// extra key, forcing a deliberate, audited edit.
export const SNOMED_PUBLIC_ALLOWLIST = Object.freeze(['sid_s', 'sid_c']);

/**
 * Project one stamped (internal, full) SNOMED concept down to its public shape.
 *
 * Pure: constructs a new object; never mutates the input. Reads ONLY the two
 * allowlisted fields. A concept missing sid_s/sid_c yields that key as `undefined`
 * (the caller / stamper invariant guarantees both are present post-stamp; the
 * projection itself never invents a value, never copies any other field).
 *
...
export function projectSnomedPublic(concept) {
    ...
    // Explicit field-by-field construction (allowlist). Do NOT spread / Object.assign /
    // pick-by-omit -- those are denylist shapes that leak new fields by default.
    return {
        sid_s: c.sid_s,
        sid_c: c.sid_c,
    };
