/**
 * UMLS LOINC (SAB=LNC) bulk harvest orchestrator (PR-UMLS-4; thin -- heavy logic in
 * lib/umls-concept-streams.js, cloned from umls-snomed-harvest.js).
 *
 * NON-CONFLATION NOTE: this LOINC axis is the UMLS LNC *concept vocabulary* (lab/clinical
 * observation codes). It is DISTINCT from lib/spl-parser.js LOINC_SECTIONS, which maps SPL
 * document-section LOINC codes to DrugLabel fields -- a different axis; the two never join.
 *
 ...
 * 'LNC'). MUST STREAM: only the byCode Map + 3 distinct-CODE Sets stay resident -- the
 * 2.34GB body is NEVER buffer-loaded. Emits one JSONL line per distinct LNC CODE (the
 * distinct-LNC count is MEASURED at harvest -- upper bound 306,528, NOT assumed; de-risked
 * < 1e6 -> NO sharding), zstd-compresses, uploads to R2, writes the cursor with the 3-SAB
 * distinct-CODE telemetry.
 *
 * LICENSE FENCE (RULING 2): this FULL artifact carries the LOINC STR + raw CODE + CUI, so it
 * is written under the `internal/` R2 prefix -- NEVER a publicly-servable path. The PUBLIC
 * snapshot exposes ONLY the Cat-0 public projection {sid_s,sid_c,code,str} (LOINC code+str
 * are redistributable at no cost under the Regenstrief license; CUI is the NLM-proprietary
 * identifier that is dropped). Bucket-policy/IAM on the internal/ prefix is a founder
 * dashboard action; this script controls only the R2 KEY PREFIX it writes to.
 *
 * Usage: node umls-loinc-harvest.js --probe-json=/tmp/umls-probe.json [--dry-run]
...
import {
    MRCONSO_COLUMNS, LOINC_SAB, LOINC_CANONICALIZATION_VERSION, makeRrfParser,
    newConceptAccumulator, ingestMrconsoRow, finalizeConcepts, buildLoincLicenseMetadata,
} from './lib/umls-concept-streams.js';

const CURSOR_KEY = 'state/umls-loinc-bulk-cursor.json';
...
async function streamMrconso(tmpZip) {
    ...
    try {
        ...
        console.log(`[LOINC-HARVEST] MRCONSO.RRF entry=${target.name} uncompressed_bytes=${target.size} (${(target.size / 1e9).toFixed(2)} GB) -- streaming`);
        const stream = await zip.stream(target.name);
        const parser = stream.pipe(makeRrfParser(MRCONSO_COLUMNS));
        for await (const row of parser) {
            ingestMrconsoRow(acc, row, LOINC_SAB);
            if (++rows % 2_000_000 === 0) console.log(`[LOINC-HARVEST] streamed ${rows} rows; LOINC concepts so far=${acc.byCode.size}`);
        }
    } finally {
        await zip.close();
    }
    console.log(`[LOINC-HARVEST] stream complete: ${rows} MRCONSO rows scanned`);
    return finalizeConcepts(acc, LOINC_SAB, LOINC_CANONICALIZATION_VERSION);
}
...
// PM-mandated 3-SAB distinct-CODE telemetry + the no-shard ceiling check (LNC branch).
// The distinct-LNC count is MEASURED here (not assumed); upper bound 306,528.
function logDistinctCodeTelemetry(d) {
    console.log(`[LOINC-HARVEST] distinct-CODE per SAB: MSH=${d.MSH} SNOMEDCT_US=${d.SNOMEDCT_US} LNC=${d.LNC}`);
    const pass = d.LNC < 1e6;
    console.log(`[LOINC-HARVEST] ceiling-check LNC distinct<1e6 ? ${pass ? 'PASS' : 'FAIL'} (no-shard precondition; measured=${d.LNC})`);
}
...
async function main() {
    ...
    console.log(`[LOINC-HARVEST] release=${release} inner_url=${inner_url}`);

    const tmpZip = join(tmpdir(), `umls-${Date.now()}.zip`);
    const tmpJsonl = join(tmpdir(), `umls-loinc-${Date.now()}.jsonl`);
    const tmpZst = `${tmpJsonl}.zst`;

    try {
        const zipBytes = await downloadZip(inner_url, tmpZip);
        console.log(`[LOINC-HARVEST] downloaded zip=${zipBytes} bytes (${(zipBytes / 1e9).toFixed(2)} GB)`);

        const { concepts, distinctCodeBySab } = await streamMrconso(tmpZip);
        console.log(`[LOINC-HARVEST] LOINC concepts (distinct LNC CODEs measured)=${concepts.length}`);
        logDistinctCodeTelemetry(distinctCodeBySab);

        const ingestionDate = new Date().toISOString().slice(0, 10);
        const licenseMetadata = buildLoincLicenseMetadata(release, ingestionDate);
        ...
        console.log(`[LOINC-HARVEST] jsonl=${jsonlBytes} bytes -> zst=${zstBytes} bytes`);

        if (args.dryRun) {
            console.log('[LOINC-HARVEST] dry-run; skipping R2 upload + cursor write');
            return;
        }
        const client = makeR2Client();
        const bucket = process.env.R2_BUCKET;
        // RULING 2: FULL STR+CODE+CUI artifact -> internal/ prefix (never publicly servable).
        const dataKey = `internal/processed/bulk/umls/${release}/loinc-concepts.jsonl.zst`;
        ...
        console.log(`[LOINC-HARVEST] R2 upload OK: ${dataKey} + ${CURSOR_KEY}`);
    } finally {
...
if (isDirectRun) main().catch(err => { console.error('[LOINC-HARVEST] FATAL:', err.message); process.exit(2); });
