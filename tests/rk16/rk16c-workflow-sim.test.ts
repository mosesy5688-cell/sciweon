// @ts-nocheck
/**
 * RK-16C TWO-MANIFEST PREFLIGHT WORKFLOW — FAKE-CLIENT SIMULATION (check 16).
 *
 ...
 * 'preflight-execute' and -- driven through preflightManifest with FAKE deps --
 * reads ONLY the two metadata objects (root seal + deterministic per-file
 * manifest sibling), touching NO payload key and NO List command, and yields the
 * payload pins for an UNRATIFIED candidate.
 *
 * It exercises preflightManifest directly (not runPreflight), so NO file is
 * written to the shared results/ candidate path. FAKE client ONLY. ZERO network.
 * NEVER makeR2Client. NEVER production R2.
 */
...
describe('check 16 — workflow command maps to preflight-execute -> two-manifest metadata-only', () => {
    ...
    it('preflightManifest reads seal+manifest.json HEAD/GET ONLY, no payload, no List; pins extracted', async () => {
        const args = parseArgs(workflowCommandArgv());
        ...
        expect(pf.root_manifest_key).toBe(SEAL);
        expect(pf.file_manifest_key).toBe(FILE_KEY);
        expect(pf.list_attempt_count).toBe(0);
        expect(pf.non_allowlisted_key_attempt_count).toBe(0);
        expect(pf.candidate.payload_compressed_bytes).toBe(62914560);
        expect(pf.candidate.payload_sha256_compressed).toBe('b'.repeat(64));
        expect(pf.candidate.expected_row_count).toBe(475112);
    });

    it('fake-run network access = exactly 4 metadata reads; zero payload bytes', async () => {
        const args = parseArgs(workflowCommandArgv());
        const ctx = fakeDeps({ [SEAL]: sealBody(), [FILE_KEY]: fileBody() });
        await preflightManifest({ execute: true, snapshot: args.snapshot, manifestKey: args.manifestKey }, ctx.deps);
